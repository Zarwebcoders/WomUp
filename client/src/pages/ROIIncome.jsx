    ...
    useEffect(() => {
        const fetchLogs = async () => {
            try {
                ...
                const { data } = await axios.get(`${API_URL}/api/income/roi?search=${searchTerm}`, config);
                setLogs(data);
                setLoading(false);
            } catch (err) {
                console.error(err);
        ...
        >
            <div className="flex justify-end">
                <div className="glass-card p-6 bg-gradient-to-br from-card to-success/10 border-success/20 min-w-[240px]">
                    <p className="text-gray-400 text-sm mb-1">Total ROI Income</p>
            ...
            <div className="glass-card overflow-hidden">
                <div className="p-6 border-b border-white/5">
                    <div className="relative w-full max-w-md">
                        ...
                        <input
                            type="text"
                            placeholder="Search by amount or month..."
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        ...
                        <tbody className="text-sm">
                            {loading ? (
                                <tr><td colSpan="5" className="px-8 py-10 text-center">Loading ROI logs...</td></tr>
                                ...
                                <tr key={log._id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                    <td className="px-8 py-4">
                                        <code className="text-primary-light bg-primary/10 px-2 py-1 rounded text-xs">
                                            #ROI-{log._id.substring(18).toUpperCase()}
                                        </code>
                                    </td>
                                    <td className="px-8 py-4 flex items-center space-x-3">
                                        <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center"><TrendingUp size={14} className="text-primary" /></div>
                                        <div>
                                            <p className="font-medium text-white">Monthly ROI</p>
                                            <p className="text-[10px] text-gray-500 uppercase tracking-tighter">Automatic Distribution</p>
                                        </div>
                                    </td>
                                    <td className="px-8 py-4">
